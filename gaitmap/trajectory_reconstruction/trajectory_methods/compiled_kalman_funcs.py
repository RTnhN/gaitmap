...
@cc.export(
    "_rts_kalman_correction_pass",
    "Tuple((float64[:, :], float64[:, :], float64[:, :]))(float64[:, :], float64[:, :], float64[:, :], float64[:, :])",
)
def _rts_kalman_correction_pass(
    positions, velocities, orientations, corrected_error_states
):
...
def _rts_kalman_backward_pass(
    ...
    for i in range(len(prior_error_states) - 2, -1, -1):
        ...
        gain = (
            posterior_covariances[i]
            @ transition_matrix.T
            @ np.linalg.inv(prior_covariances[i + 1])
        )
        ...
        corrected_covariances[i, :, :] = (
            posterior_covariances[i]
            + gain @ (corrected_covariances[i + 1] - prior_covariances[i + 1]) @ gain.T
        )

    return corrected_error_states, corrected_covariances


def _rts_kalman_update_series_fast(
    acc, gyro, initial_orientation, sampling_rate_hz, meas_noise, process_noise, zupts
):
    forward_eskf_results, forward_nominal_states = default_rts_kalman_forward_pass_fast(
        acc,
        gyro,
        initial_orientation,
        sampling_rate_hz,
    ...
    corrected_states = _rts_kalman_correction_pass(
        *forward_nominal_states, corrected_error_states
    )
...
# )
# def simple_navigation_equations(
#     acc, gyro, orientation, position, velocity, sampling_rate_hz
# ):
...
#     new_position = (
#         position
#         + velocity / sampling_rate_hz
#         + 0.5 * (rotated_acc - GRAV_VEC) / sampling_rate_hz**2
#     )
...
@cc.export(
    "default_rts_kalman_forward_pass_fast",
    "Tuple((Tuple((float64[:, :, :], float64[:, :, :], float64[:, :], float64[:, :], float64[:, :, :])), Tuple((float64[:, :], float64[:, :], float64[:, :]))))(float64[:, :], float64[:, :], float64[:], float64, float64[:, :], float64[:, :], bool[:])",
)
def default_rts_kalman_forward_pass_fast(  # pylint: disable=too-many-statements  # noqa: PLR0915
    ...
    for i, zupt in enumerate(zupts):
        ...
        position, velocity, orientation = simple_navigation_equations(
            acc,
            omega,
            orientations[i],
            positions[i],
        ...
        transition_matrix[3:6, 6:] = (
            -cross_product_matrix(rotated_acc) / sampling_rate_hz
        )
        ...
        prior_covariances[i + 1, :, :] = (
            transition_matrix @ posterior_covariances[i] @ transition_matrix.T
            + process_noise
        )
        ...
        if zupt:
            ...
            if level_walking:
                # z-position error
                zupt_measurement[3] = position[
                    2
                ]  # not in paper but would be \hat{p_z}_n
            ...
            innovation_cov = (
                meas_jacob @ prior_covariances[i + 1] @ meas_jacob.T + meas_noise
            )
            ...
            gain = (
                prior_covariances[i + 1] @ meas_jacob.T @ np.linalg.pinv(innovation_cov)
            )
            ...
            posterior_error_states[i + 1, :] = (
                prior_error_states[i + 1] + gain @ innovation
            )
            ...
            posterior_covariances[i + 1, :, :] = (
                covariance_adj + gain @ meas_noise @ gain.T
            )
        else:
